...
class TodoProjectBindingTests(TestCase):
    ...
    def test_add_task_with_existing_project(self):
        ...
        task = TodoItem.objects.get(owner=self.user, title="TodoItem con progetto")
    ...
    def test_add_task_creates_new_project_if_requested(self):
        ...
        self.assertEqual(response.status_code, 302)
        task = TodoItem.objects.get(owner=self.user, title="TodoItem nuovo progetto")
    ...
    def test_add_task_as_reminder(self):
        ...
        self.assertEqual(response.status_code, 302)
        task = TodoItem.objects.get(owner=self.user, title="Ricordati rinnovo")
    ...
    def test_add_task_as_appointment(self):
        ...
        response = self.client.post(
            "/todo/api/add",
            {
                ...
                "item_type": TodoItem.ItemType.APPOINTMENT,
                "due_date": "2026-03-02",
                "due_time": "15:00",
                "status": TodoItem.Status.OPEN,
                "priority": TodoItem.Priority.MEDIUM,
        ...
        self.assertEqual(response.status_code, 302)
        task = TodoItem.objects.get(owner=self.user, title="Call cliente ore 15")
        self.assertEqual(task.item_type, TodoItem.ItemType.APPOINTMENT)
    ...
    def test_add_task_with_existing_category(self):
        ...
        self.assertEqual(response.status_code, 302)
        task = TodoItem.objects.get(owner=self.user, title="Follow-up cliente")
    ...
    def test_add_task_creates_new_category_if_requested(self):
        ...
        response = self.client.post(
            "/todo/api/add",
            {
                "title": "TodoItem con nuova categoria",
                "item_type": TodoItem.ItemType.TASK,
                "status": TodoItem.Status.OPEN,
                "priority": TodoItem.Priority.LOW,
                "project_choice": "",
        ...
        self.assertEqual(response.status_code, 302)
        task = TodoItem.objects.get(owner=self.user, title="TodoItem con nuova categoria")
    ...
    def test_set_status_htmx_returns_oob_fragment(self):
        ...
        response = self.client.post(
            "/todo/api/status",
            {"id": str(task.id), "status": TodoItem.Status.DONE},
        ...
        self.assertEqual(task.status, TodoItem.Status.DONE)

    def test_set_status_non_htmx_redirects(self):
        task = TodoItem.objects.create(
            owner=self.user,
            title="TodoItem status redirect",
            item_type=TodoItem.ItemType.TASK,
            status=TodoItem.Status.OPEN,
            priority=TodoItem.Priority.LOW,
        )
        ...
        response = self.client.post(
            "/todo/api/status",
            {"id": str(task.id), "status": TodoItem.Status.IN_PROGRESS},
        )
        self.assertEqual(response.status_code, 302)
        self.assertEqual(response.url, "/todo/")
        task.refresh_from_db()
        self.assertEqual(task.status, TodoItem.Status.IN_PROGRESS)
