...
def _todo_counts(user, today=None):
    if today is None:
        today = date.today()
    base_qs = TodoItem.objects.filter(owner=user)
    return {
        "open": base_qs.filter(status=TodoItem.Status.OPEN).count(),
        "in_progress": base_qs.filter(status=TodoItem.Status.IN_PROGRESS).count(),
        "done": base_qs.filter(status=TodoItem.Status.DONE).count(),
        "overdue": base_qs.exclude(status=TodoItem.Status.DONE).filter(due_date__lt=today).count(),
        "today": base_qs.exclude(status=TodoItem.Status.DONE).filter(due_date=today).count(),
    }
...
def dashboard(request):
    ...
    today = date.today()
    task_rows = (
        TodoItem.objects.filter(owner=user)
        .select_related("project", "category")
        .annotate(
            status_rank=Case(
                When(status=TodoItem.Status.OPEN, then=Value(0)),
                When(status=TodoItem.Status.IN_PROGRESS, then=Value(1)),
                default=Value(2),
...
def remove_task(request):
    ...
    if task_id:
        ...
        if request.method == "POST":
            ...
            task.delete()
            return redirect("/todo/")
    tasks = TodoItem.objects.filter(owner=request.user).order_by("-created_at")[:20]
...
def update_task(request):
    ...
    if task_id:
        task = get_object_or_404(Task, id=task_id, owner=request.user)
        if request.method == "POST":
            form = TodoItemForm(request.POST, instance=task, owner=request.user)
            if form.is_valid():
                task = form.save()
                _sync_task_quiet(task)
                return redirect("/todo/")
        else:
            form = TodoItemForm(instance=task, owner=request.user)
...
def set_status(request):
    ...
    allowed_statuses = {TodoItem.Status.OPEN, TodoItem.Status.IN_PROGRESS, TodoItem.Status.DONE}
