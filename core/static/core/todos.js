...
withStimulusModule(({ Controller }) => {
  class TodosController extends Controller {
    connect() {
      ...
      this.todayStatsRoot = this.element.querySelector(".todos-today-stats");
    ...
    applyFilters() {
      ...
      this.element.querySelectorAll("[data-routine-day]").forEach((day) => {
        ...
        cards.forEach((row) => {
          ...
          row.classList.toggle("todos-hidden", !visible);
          if (visible) {
            visibleCards += 1;
          }
        });
        ...
        day.classList.toggle("todos-hidden", !showDay);
      });
    ...
    syncActiveDayButton() {
      const visibleDay = Array.from(this.daySections).find((day) => !day.classList.contains("todos-hidden"));
      if (visibleDay) {
  ...
  class TasksController extends Controller {
    ...
    connect() {
      this.listTarget.addEventListener("change", (e) => {
        if (e.target.matches("[data-task-status]")) {
          this._updateStatus(e.target);
        }
      });
      this.listTarget.addEventListener("click", (e) => {
        if (e.target.matches("[data-action^='tasks#']")) {
          const action = e.target.dataset.action.replace("tasks#", "");
        ...
        if (form) {
          form.addEventListener("htmx:after-request", (e) => {
            if (e.detail.successful) {
              const resp = JSON.parse(form.dataset._lastResponse || "{}");
              if (resp.ok) {
    ...
    async deleteTask(button) {
      ...
      fd.append("id", id);
      try {
        const resp = await fetch("/todos/api/task/remove", {
          method: "POST",
    ...
    async _updateStatus(select) {
      ...
      try {
        const resp = await fetch("/todos/api/task/status", {
          method: "POST",
          headers: { "X-CSRFToken": this._csrf() },
          body: fd,
        });
        const json = await resp.json();
        if (!json.ok) {
          notifyError("Stato non aggiornato");
        }
      } catch (err) {
        notifyError("Errore durante l'aggiornamento");
      }
    }
  ...
  registerStimulusController("todos", TodosController);
  registerStimulusController("tasks", TasksController);
}).catch(() => {});
