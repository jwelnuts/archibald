...
define("dojox/widget/MultiSelectCalendar", [
    ...
    "dojo/date", "dojo/date/stamp",
    "dojo/date/locale",
    "dijit/_Widget", "dijit/_TemplatedMixin", "dijit/_WidgetsInTemplateMixin",
	"dijit/_CssStateMixin", "dijit/form/DropDownButton", "dijit/typematic"
],
    function(dojo, dijit, template, supplemental, date, stamp, locale,
	...
	[_Widget, _TemplatedMixin, _WidgetsInTemplateMixin, _CssStateMixin],
	{
			...
			if(dojo.isArray(value)) {
				dojo.forEach(value,function(element, i){
					...
					if(slashPosition == -1){
						...
						var dateA = stamp.fromISOString(element.substr(0,10));
						var dateB = stamp.fromISOString(element.substr(11,10));
			...
			if(value.length > 0){
				...
				if(this._isValidDate(value)){
					...
					if(!this.isDisabledDate(value, this.lang)){
						dateIndex = stamp.toISOString(value).substring(0,10);
		...
		focusOnLastDate : function(lastElement){
			...
			if(slashPositionLastDate == -1){
				...
				lastDate = stamp.fromISOString(lastElement);
			}else{
				dateA = stamp.fromISOString(lastElement.substr(0,10));
				dateB = stamp.fromISOString(lastElement.substr(11,10));
				if((dateA - dateB) > 0){
					lastDate = dateA;
				}else{
					lastDate = dateB;
		...
		_populateGrid: function(){
			...
			this.listOfNodes.forEach(function(template, i){
				...
				dateIndex = stamp.toISOString(date).substring(0,10);
		...
		toggleDate : function(/*date*/ dateToToggle, /*array of dates*/ selectedDates, /*array of dates*/ unselectedDates){
			...
			var dateIndex = stamp.toISOString(dateToToggle).substring(0,10);
		...
		selectDate : function(/*date*/ dateToSelect, /*array of dates*/ selectedDates){
			...
			var dateIndex = stamp.toISOString(dateToSelect).substring(0,10);
			this.value[dateIndex] = 1;
		...
		unselectDate : function(/*date*/ dateToUnselect, /*array of dates*/ unselectedDates){
			...
			var clazz = node.className;
			var dateIndex = stamp.toISOString(dateToUnselect).substring(0,10);
			...
			//selected
				...
				if(node && !dojo.hasClass(node, "dijitCalendarDisabledDate")){
					...
					if(!this.rangeJustSelected){
						...
						this.set("currentFocus", value);
						this.onValueSelected([stamp.toISOString(value).substring(0,10)]);
...
//TODO: use typematic
			...
			switch(evt.keyCode){
				...
				case dk.SPACE:
					if(evt.shiftKey && this.previouslySelectedDay){
						...
						this.previousRangeStart = null;
						this.previousRangeEnd = null;
						this.onValueSelected([stamp.toISOString(newValue).substring(0,10)]);
	...
	//In this method we remove some dates from a range from left to right
			...
			for(var i = 0; i <= difference; i++){
				var nextDay = date.add(beginning, 'day',i);
            ...
            // FIXME: nextDay is not defined...
			this.previouslySelectedDay = date.add(nextDay, 'day',1);
		},
		...
		_addToRangeRTL : function(/*date*/ beginning, /*date*/ end, /*array*/selectedDates, /*array*/unselectedDates){
			...
			difference = Math.abs(date.difference(beginning, end, "day"));
			//If the end of the range is earlier than the beginning (back in time),
			//we are going to start from the end and move backward
		...
		_isSelectedDate: function(/*Date*/ dateObject, /*String?*/ locale){
			...
			//		Returns true if the passed date is part of the selected dates of the calendar
				...
				dateIndex = stamp.toISOString(dateObject).substring(0,10);
...
=====*/
		...
		if(selectedDates.length > 1){
			...
			var weHaveRange = false,
				...
				startRange = null,
				lastDayRange = null,
				previousDate = stamp.fromISOString(selectedDates[0]);

			for(var i = 1; i < selectedDates.length+1; i++){
				currentDate = stamp.fromISOString(selectedDates[i]);
				if(weHaveRange){
				//We are in the middle of a range
					difference = Math.abs(date.difference(previousDate, currentDate, "day"));
					if(difference == 1){
						//we continue with the range
						lastDayRange = currentDate;
					}else{
						//end of the range, reset variables for maybe the next range..
						range = stamp.toISOString(startRange).substring(0,10) +
								"/" + stamp.toISOString(lastDayRange).substring(0,10);
					...
					if(difference == 1){
						...
						returnDates.push(stamp.toISOString(previousDate).substring(0,10));
					}
				}
